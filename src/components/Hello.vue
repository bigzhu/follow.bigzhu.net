...
<script>
  ...
  const {
    ...
    position
  } = event, moveForce = 30, rotateForce = 40, RAD_TO_DEG = 180 / Math.PI
  ...
  function getRotationFromAccel(accelX, accelY, accelZ) {
    ...
    return {
      roll: Math.atan2(accelY, sign * Math.sqrt(Math.pow(accelZ, 2) + miu *
        Math.pow(accelX, 2))) * RAD_TO_DEG,
      pitch: -Math.atan2(accelX, Math.sqrt(Math.pow(accelY, 2) + Math.pow(
        accelZ, 2))) * RAD_TO_DEG
    }
  }
      ...
      rotate(evt) {
        if (evt.rotationRate &&
          ...
          const
            ...
            accelZ = evt.acceleration.z || evt.accelerationIncludingGravity.z -
            9.81,
